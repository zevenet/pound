...
int body_max_size;
#if WAF
WAF_RULESET_MEMO *waf_rules_memo;       // last waf ruleset loaded
pthread_mutex_t waf_rules_memo_mtx;     // lock the waf resource
ModSecurity *waf_api;
...
static void l_init(void)
{
  ...
  if ((l_array =
       (pthread_mutex_t *) calloc(n_locks, sizeof(pthread_mutex_t))) == NULL) {
...
static void l_lock(const int mode, const int n, /* unused */ const char *file,  /* unused */
                   int line)
{
  int ret_val;
...
static unsigned long l_id(void)
{
...
thr_arg *get_dyn_thr_arg(thr_arg * arg)
{
  thr_arg *res;
 ...
 */
static RETSIGTYPE h_term(const int sig)
{
 ...
 */
static RETSIGTYPE h_shut(const int sig)
{
  int status;
  LISTENER *lstn;
...
void zcu_bt_print(void)
{
    ...
    for (i = 0; i < calls; i++)
        logmsg(LOG_ERR, "Backtrace_symbol: %s", str[i]);
...
int main(const int argc, char **argv)
{
  ...
  if (ctrl_name != NULL) {
    ...
    if (bind(control_sock, (struct sockaddr *) &ctrl, (socklen_t) sizeof(ctrl))
        < 0) {
    ...
    if (ctrl_user) {
      ...
      if (chown(ctrl_name, pw->pw_uid, -1)) {
        logmsg(LOG_ERR, "chown error on control socket - aborted (%s)",
               strerror(errno));
        exit(1);
      }
    }
    ...
    if (ctrl_mode > 0) {
      if (chmod(ctrl_name, ctrl_mode)) {
        logmsg(LOG_ERR, "chmod error on control socket - aborted (%s)",
               strerror(errno));
        exit(1);
      }
    }
  ...
  for (lstn = listeners, n_listeners = 0; lstn;
    ...
    setsockopt(lstn->sock, SOL_SOCKET, SO_REUSEADDR, (void *) &opt,
               sizeof(opt));
    if (bind(lstn->sock, lstn->addr.ai_addr, (socklen_t) lstn->addr.ai_addrlen)
        < 0) {
      addr2str(tmp, MAXBUF - 1, &lstn->addr, 0);
  ...
  if ((polls =
       (struct pollfd *) calloc(n_listeners, sizeof(struct pollfd))) == NULL) {
  ...
  for (;;) {
    ...
    if ((son = fork()) > 0) {
      ...
      if (WIFEXITED(status))
        ...
        logmsg(LOG_ERR, "MONITOR: worker exited on signal %d, restarting...",
               WTERMSIG(status));
      else
        logmsg(LOG_ERR, "MONITOR: worker exited (stopped?) %d, restarting...",
               status);
    } else if (son == 0) {
#endif
        ...
        if (poll(polls, n_listeners, -1) < 0) {
          ...
          for (lstn = listeners, i = 0; lstn; lstn = lstn->next, i++) {
            if (polls[i].revents & (POLLIN | POLLPRI)) {
              ...
              if ((clnt = accept(lstn->sock, (struct sockaddr *) &clnt_addr,
              ...
              } else if (((struct sockaddr_in *) &clnt_addr)->sin_family ==
                         AF_INET
                         || ((struct sockaddr_in *) &clnt_addr)->sin_family ==
                         AF_INET6) {
                thr_arg arg, *argp;
                ...
                if ((arg.from_host.ai_addr =
                     (struct sockaddr *) malloc(clnt_length)) == NULL) {
                ...
                if (threadpool) {
                  ...
                  if ((argp = get_dyn_thr_arg(&arg)) == NULL)
                    ...
                    if (pthread_create
                        (&thr, &attr, thr_http_single, (void *) argp)) {
