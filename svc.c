 ...
 */
void
t_add(SERVICE * const srv, const char *key, const void *content,
      const size_t cont_len, unsigned long timestamp)
{
  if (strlen(key) <= 0) {
    logmsg(LOG_WARNING, "Session key not valid; Key %s len %d", key,
           strlen(key));
    return;
  }
  TABNODE *t, *old;
  ...
  if (sync_is_enabled)
    notify(SESS_ADD, srv->listener_key_id, srv->key_id, t->key, t->content,
           (unsigned int) (t->last_acc));
...
void *t_find(SERVICE * const srv, char *const key)
{
  ...
  if ((res = (TABNODE *) lh_retrieve(tab, &t)) != NULL) {
    ...
    if (res->last_acc != time(NULL)) {
      ...
      //TODO:: SEND UPDATE
      if (sync_is_enabled)
        notify(SESS_UPDATE, srv->listener_key_id, srv->key_id, res->key,
               res->content, (unsigned int) (res->last_acc));
    }
    return res->content;
  }
  return NULL;
...
void t_remove(SERVICE * const srv, char *const key)
{
  ...
  if ((res = (TABNODE *) lh_delete(tab, &t)) != NULL) {
    ...
    //TODO: NOTIFY ZLB
    if (sync_is_enabled)
      notify(SESS_DELETE, srv->listener_key_id, srv->key_id, res->key,
             res->content, (unsigned int) (res->last_acc));
...
static void t_old_doall_arg(TABNODE * t, ALL_ARG * a)
{
  TABNODE *res;
...
#endif
      if (sync_is_enabled)
        notify(SESS_DELETE, res->listener, res->service, res->key, res->content,
               (unsigned int) (res->last_acc));
      free(res->key);
      free(res->content);
      free(res);
    }
 ...
 */
static void t_expire(LHASH_OF(TABNODE) * const tab, const time_t lim)
{
  ALL_ARG a;
  int down_load;
...
static void t_cont_doall_arg(TABNODE * t, ALL_ARG * arg)
{
  TABNODE *res;
  ...
  if (log_facility == -1) {
    if (name)
      fprintf((priority == LOG_INFO
               || priority == LOG_DEBUG) ? stdout : stderr, "%s, %s\n", name,
              buf);
    else
      fprintf((priority == LOG_INFO
...
#else
void logmsg(const int priority, const char *fmt, va_alist)
va_dcl
{
  char buf[MAXBUF + 1];
  va_list ap;
...
void
addr2IpAndPort(char *const res, int *return_port, const int res_len,
 ...
 */
int cpURL(char *res, char *src, int len)
{
  int state;
  char *kp_res;
 ...
 */
int check_header(const char *header, char *const content)
{
 ...
 */
SERVICE *get_service(const LISTENER * lstn, const char *request,
                     char **const headers)
{
  SERVICE *svc;
 ...
 */
static int get_REQUEST(char *res, const SERVICE * svc, const char *request)
{
  int n, s;
  regmatch_t matches[4];
...
static int get_HEADERS(char *res, const SERVICE * svc, char **const headers)
{
  int i, n, s;
  regmatch_t matches[4];
 ...
 */
static BACKEND *rand_backend(BACKEND * be, int pri)
{
  while (be) {
...
static BACKEND *get_backend_by_key(BACKEND * be, const char *bekey)
{
  if (!bekey || !*bekey)
    return NULL;
  while (be) {
    if (be->bekey && strcmp(be->bekey, bekey) == 0)
      return be;
    be = be->next;
  }
  return NULL;
 ...
 */
static BACKEND *hash_backend(BACKEND * be, int abs_pri, char *key)
{
  unsigned long hv;
  BACKEND *res, *tb;
  int pri;
 ...
 */
BACKEND *get_backend(SERVICE * const svc, const struct addrinfo *from_host,
                     const char *request, char **const headers,
                     int listener_log_level)
{
  BACKEND *res;
  char key[KEY_SIZE + 1];
  ...
  switch (svc->sess_type) {
      ...
      if (no_be)
        ...
        if (res == NULL || !res->alive || res->disabled) {
          ...
          if (res != NULL && listener_log_level > 1) {
            ...
            logmsg(LOG_DEBUG,
                   "service %s, No backend found for BEKEY %s found in headers, assigned backend %s",
                   svc->name, bekey, bk);
          }
        } else if (listener_log_level > 1) {
          str_be(bk, MAXBUF - 1, res);
          logmsg(LOG_DEBUG, "service %s, found matching backend %s by bekey %s",
                 svc->name, bk, bekey);
        }
    ...
    case SESS_IP:
      ...
      if (svc->sess_ttl < 0) {
        res =
          no_be ? svc->emergency : hash_backend(svc->backends, svc->abs_pri,
                                                key);
        ...
        if (no_be)
          ...
          if (get_bekey_from_HEADERS(bekey, svc, headers)) {
            ...
            if (res == NULL || !res->alive || res->disabled) {
              ...
              str_be(bk, MAXBUF - 1, res);
              logmsg(LOG_DEBUG,
                     "service %s, found matching backend %s by bekey %s",
                     svc->name, bk, bekey);
            }
          } else
            res = rand_backend(svc->backends, random() % svc->tot_pri);
    ...
    case SESS_PARM:
      if (get_REQUEST(key, svc, request)) {
        ...
        res =
          no_be ? svc->emergency : rand_backend(svc->backends,
                                                random() % svc->tot_pri);
      }
      break;
    default:
 ...
 */
void upd_session(SERVICE * const svc, char **const headers, BACKEND * const be)
{
  char key[KEY_SIZE + 1];
  int ret_val;
 ...
 */
void kill_be(SERVICE * const svc, const BACKEND * be, const int disable_mode)
{
  BACKEND *b;
  int ret_val;
  char buf[MAXBUF];
 ...
 */
void upd_be(SERVICE * const svc, BACKEND * const be, const double elapsed)
{
  int ret_val;
             ...
             const char *v_host, const LISTENER * lstn, const BACKEND * be)
{
  ...
  if (addr.ai_family == AF_INET) {
     ...
     */
    if (memcmp
        (&be6_addr.sin6_addr.s6_addr, &in6_addr.sin6_addr.s6_addr,
           ...
           char *buf_log_tag)
{
  ...
  if ((flags = fcntl(sockfd, F_GETFL, 0)) < 0) {
    logmsg(LOG_WARNING, "%s (%lx) connect_nb: fcntl GETFL failed: %s",
           buf_log_tag, pthread_self(), strerror(errno));
    return -1;
  }
  if (fcntl(sockfd, F_SETFL, flags | O_NONBLOCK) < 0) {
    logmsg(LOG_WARNING, "%s (%lx) connect_nb: fcntl SETFL failed: %s",
           buf_log_tag, pthread_self(), strerror(errno));
    return -1;
  }
  ...
  if ((res = connect(sockfd, serv_addr->ai_addr, serv_addr->ai_addrlen)) < 0)
    if (errno != EINPROGRESS) {
      logmsg(LOG_WARNING, "%s (%lx) connect_nb: connect failed: %s",
             buf_log_tag, pthread_self(), strerror(errno));
      return (-1);
    }
  ...
  if (res == 0) {
    ...
    if (fcntl(sockfd, F_SETFL, flags) < 0) {
      logmsg(LOG_WARNING, "%s (%lx) connect_nb: fcntl reSETFL failed: %s",
             buf_log_tag, pthread_self(), strerror(errno));
      return -1;
    }
    return 0;
  ...
  if ((res = poll(&p, 1, to * 1000)) != 1) {
    if (res == 0) {
      /* timeout */
      logmsg(LOG_WARNING, "%s (%lx) connect_nb: poll timed out", buf_log_tag,
             pthread_self());
      errno = ETIMEDOUT;
    } else
      logmsg(LOG_WARNING, "%s (%lx) connect_nb: poll failed: %s", buf_log_tag,
  ...
  if (getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
    logmsg(LOG_WARNING, "%s (%lx) connect_nb: getsockopt failed: %s",
           buf_log_tag, pthread_self(), strerror(errno));
    return -1;
  }
  ...
  if (error) {
    ...
    errno = error;
    logmsg(LOG_WARNING, "%s (%lx) connect_nb: error after getsockopt: %s",
           buf_log_tag, pthread_self(), strerror(errno));
    return -1;
  }
      ...
      if (modified) {
        ...
        for (be = svc->backends; be; be = be->next) {
          if (be->resurrect) {
            ...
            logmsg(LOG_NOTICE,
                   "BackEnd %s resurrect in farm: '%s', service: '%s'", buf,
                   name, svc->name);
                      ...
                      int keylength)
{
  ...
  res =
    (keylength <=
     512) ? RSA512_keys[rand() % N_RSA_KEYS] : RSA1024_keys[rand() %
                                                            N_RSA_KEYS];
...
static void t_dump_doall_arg(TABNODE * t, DUMP_ARG * arg)
{
  BACKEND *be, *bep;
  int n_be, sz;
 ...
 */
static LISTENER *sel_lstn(const CTRL_CMD * cmd)
{
  LISTENER *lstn;
  int i;
 ...
 */
static SERVICE *sel_svc(const CTRL_CMD * cmd)
{
  SERVICE *svc;
  LISTENER *lstn;
  int i;
 ...
 */
static BACKEND *sel_be(const CTRL_CMD * cmd)
{
  BACKEND *be;
  SERVICE *svc;
  int i;
    ...
    switch (cmd.cmd) {
        ...
        for (lstn = listeners; lstn; lstn = lstn->next) {
          ...
          for (svc = lstn->services; svc; svc = svc->next) {
            ...
            for (be = svc->backends; be; be = be->next) {
              ...
              if (sz > 0)
                (void) write(ctl, (void *) be->bekey, sz);
      ...
      case CTRL_EN_BE:
        ...
        if ((be = sel_be(&cmd)) == NULL)
          logmsg(LOG_INFO, "thr_control() bad backend %d/%d/%d", cmd.listener,
                 cmd.service, cmd.backend);
        else
      ...
      case CTRL_ADD_SESS:
        ...
        if ((be = sel_be(&cmd)) == NULL) {
          logmsg(LOG_INFO, "thr_control() bad back-end %d/%d", cmd.listener,
                 cmd.service);
          break;
        }
        if (ret_val = pthread_mutex_lock(&svc->mut))
